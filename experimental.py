...
import pandas as pd
import numpy as np
from tools.fangraphs_projections import get_projections
...
for id_ in player_ids[0:]:
    ...
    pitch_new_rank = pp.pre_process_pitch(new_projections_pit.reset_index())
...
# %%
data_variation_df = pd.DataFrame(data_variation).set_index("platerid")
player_list = player_list.join(data_variation_df, how="inner")
# %%
player_list.loc[:, "POSIBLE_RANGE"] = player_list.RANK_B - player_list.RANK_W
player_list.loc[:, "OVERPERFORM"]   = player_list.RANK_B - player_list.RANK
player_list.loc[:, "UNDERPERFORM"]  = player_list.RANK - player_list.RANK_W
# %%
