...
bats_mean_rank  = pp.pre_process_bat(bats_mean)#.set_index("playerid")#[["POINTS"]]
pitch_mean_rank = pp.pre_process_pitch(pitch_mean)#.set_index("playerid")#[["POINTS"]]
mean_ranks = pitch.add(bats, fill_value=0).sort_values("POINTS")
...
def generate_new_projections(means, stds, playerid, type, best_case):
    ...
    if type == "bat":
        ...
        cols_plus = ["GS", "G", "SV", "HLD", "IP", "TBF", "H", "SO"]
        cols_minus = ["R", "ER", "HR", "BB", "HBP", "BS"]
