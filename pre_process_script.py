...
def pre_process_bat(name, min_pa = 250):
    # Read in data
    df = pd.read_csv("./data/projections/{}_bats.csv".format(name))
    ...
    # Save data to csv
    df.to_csv("data/proc/{}_bats.csv".format(name))
    return None
...
def pre_process_pitch(name):
    ...
    # Read in data
    df = pd.read_csv("data/projections/{}_pitch.csv".format(name))
    df.set_index('playerid', inplace = True, drop=False)
    dic_cols = {"SO":"K"}
    df.rename(columns=dic_cols, inplace = True)
    ...
    df.to_csv("data/proc/{}_pitch.csv".format(name))
